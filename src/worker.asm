...
NUM_WORKERS: equ 12
workers_init_vals:
    DB 5*8, 8*8, 0, 16, %11, 0, 0		; worker 1
    DW SCR2_CHARPOS+(64/8*32+24/8)		; vram addr of worker 1
    DB 12*8, 8*8, 0, 32, %111, 0, 1
    DW SCR2_CHARPOS+(64/8*32+80/8)
    DB 19*8, 8*8, 0, 64, %1111, 0, 2
    DW SCR2_CHARPOS+(64/8*32+136/8)
    DB 26*8, 8*8, 0, 128, %11111, 0, 3
    DW SCR2_CHARPOS+(64/8*32+192/8)
    DB 4*8, 13*8, 0, 128, %11111, 1 << WORKER_FLAG_LEFTSIDE, 4
    DW SCR2_CHARPOS+(13*32+4)
    DB 11*8, 13*8, 0, 16, %11, 1 << WORKER_FLAG_LEFTSIDE, 5
    DW SCR2_CHARPOS+(104/8*32+88/8)
    DB 18*8, 13*8, 0, 32, %111, 1 << WORKER_FLAG_LEFTSIDE, 6
    DW SCR2_CHARPOS+(104/8*32+144/8)
    DB 25*8, 13*8, 0, 64, %1111, 1 << WORKER_FLAG_LEFTSIDE, 7
    DW SCR2_CHARPOS+(104/8*32+200/8)
    DB 5*8, 18*8, 0, 64, %1111, 0, 8
    DW SCR2_CHARPOS+(144/8*32+24/8)
    DB 12*8, 18*8, 0, 128, %11111, 0, 9
    DW SCR2_CHARPOS+(144/8*32+80/8)
    DB 19*8, 18*8, 0, 32, %111, 0, 10
    DW SCR2_CHARPOS+(144/8*32+136/8)
    DB 26*8, 18*8, 0, 16, %111, 0, 11         ; worker 12
